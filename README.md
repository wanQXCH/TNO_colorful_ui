...
2. **自定义取色器**：色相环 + 饱和度/亮度滑杆（HSV 模型——亮度 **0% = 纯黑、
   ...
   界面主色随之实时联动；
3. **生成效果预览条**：深青底/参考蓝/纸张白/纯白/灰阶文字/红色强调等 8 类典型
   TNO 界面颜色，实时显示「原图 → 换色后」——取色和压暗的组合效果看一眼便知；
4. **选项**：源图亮白压暗滑杆（暗色系风格，只压原图亮白、不影响目标色）、
   DXT5 压缩、并行进程数、Mod 显示名、生成后自动复制到 HOI4 mod 目录；
...
- **国旗/旗帜**——**旗帜图案本体绝不换色**（国旗是语义色，换色会混出诡异的绿）；
  但国旗上那层"霓虹蓝光效"（覆盖层）属于 UI 色调，**会换成目标色**：
  - 旗帜图案保护（三层）：① `gfx/flags/**` 目录整体排除（3300+ 国旗）；
    ② 文件名含 `flag` 的一切贴图；③ 任意路径段含 `flag` 的目录
    （旗帜选择界面、游击队旗帜等非标准存放）
  - **覆盖层/框架光效换色**（白名单放行）：`flag_overlay` / `flag_frame` /
    `flag_progbar` 系列贴图（外交界面、经济界面、国家视图、贸易页、顶栏、
    大型选旗界面等的霓虹蓝"罩层"光效）——低透明度蓝青渐变直接换成目标色，
    alpha 透明度保持不变，于是国旗上那层蓝光变成目标色光，整个界面色调统一
