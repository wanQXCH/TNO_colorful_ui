...
2. **自定义取色器**：色相环 + 饱和度/亮度滑杆（HSV 模型——亮度 **0% = 纯黑、
   100% = 该颜色最大亮度**，单调无歧义）+ `#RRGGBB` 手输 + 9 个预设色块，
   界面主色随之实时联动；
3. **生成效果预览条**：深青底/参考蓝/纸张白/纯白/灰阶文字/红色强调/蓝色国旗等
   9 类典型 TNO 界面颜色，实时显示「原图 → 换色后」——取色、压暗、旗帜强度的
   组合效果看一眼便知；
4. **选项**：源图亮白压暗滑杆（暗色系风格，只压原图亮白、不影响目标色）、
   **旗帜换色强度滑杆**（淡颜色滤镜，默认 40%）、DXT5 压缩、并行进程数、
   Mod 显示名、生成后自动复制到 HOI4 mod 目录；
5. **输出目录** + 「生成 Mod / 仅扫描统计」两种模式；
6. **底部固定操作坞**：开始/取消/模式切换 + 迷你进度永远可见，无需在页面里上下滚动；
   完成后显示统计卡片与 `preview.png` 原版/换色后对照图，一键打开输出文件夹。
...
# 旗帜淡颜色滤镜强度（默认 0.4；0=旗帜完全不换，1=旗帜全换色）
python tno_color_gen.py --mods "D:\...\2438003901" --color "#F5A524" --flag-blend 0.25 --out TNO_UI_Gold_soft
...
- **国策图标**（`gfx/interface/goals/**`）——整体保持原样，仅白名单例外
  ...
  **领袖头像/照片**（`gfx/leaders/**`）——保持原样
- **国旗/旗帜**——默认做**淡颜色滤镜**（换色强度 40%：换色结果与原图按比例混合，
  保留国旗辨识度；蓝/青旗全换色会混出绿等诡异色，低强度则只是叠上目标色倾向）。
  强度 0~100% 可调（Web 滑杆 / CLI `--flag-blend`）：0% = 旗帜完全不换（保持原色），
  100% = 旗帜全换色；覆盖 `gfx/flags/**`、`flag_*` 组件与任意路径段含 flag 的目录
